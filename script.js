let appId = "ae054036fe56760cd68d921af6d8ad55";
let units = "metric";
let searchMethod;

function getSearchMethod(searchTerm) {
  if (
    (searchTerm.length === 5 && Number.parseInt(searchTerm) + "") === searchTerm
  ) {
    searchMethod = "zip";
  } else searchMethod = "q";
}
...
function init(resultFromServer) {
  ...
  humidityElement.innerHTML =
    "Humidity level is at " + resultFromServer.main.humidity + "%";

  setPositionForWeatherInfo();
}

function setPositionForWeatherInfo() {
  console.log("are you even in here");
